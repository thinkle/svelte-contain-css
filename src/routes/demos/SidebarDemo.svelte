  ...
  <Container>
    <TextLayout>
      ...
      <p>
        ...
        it works in any container, see below. It ships with its own collapse affordance,
        so the simplest useful version takes no props at all: a grab bar down the
        edge when there is room, and a small round button that opens a floating sheet
        when there is not.
      </p>
    </TextLayout>
      ...
      <DemoWithCode height="420px" code={modeCode}>
        {#snippet blurb()}
          ...
          <p>
            ...
            starts shut -- a rail floating on top of your content is an odd
            thing to ask anyone to grab, and a panel that starts open over the
            content is worse. Narrow layouts always behave this way, so
    ...
    <main>Page content</main>
      ...
      >
        {#snippet blurb()}
          ...
          <p>
            ...
            element it lives in rather than the browser window -- drop one
            inside a split pane or a card and it adapts on its own. Drag the
            slider below 512px to watch the grab bar become a sheet button.
          </p>
        {/snippet}
        {#snippet inputArea()}
          <FormItem>
      ...
      <DemoWithCode height="440px" code={controlCode}>
        {#snippet blurb()}
          ...
          <p>
            <code>open</code> is bindable, so your own button can open and close
            the sidebar. It is one piece of state shared with the sidebar's built-in
            toggles -- click the grab bar and the value below changes too, so the
            two never disagree.
          </p>
          <p>
            Leave it <code>undefined</code> to keep the per-layout defaults (a rail
            starts open, a sheet starts shut). Write a boolean and that boolean wins
            in both layouts.
          </p>
        {/snippet}
        {#snippet inputArea()}
          <Bar>
            ...
            <Button onclick={() => (navOpen = undefined)}
      ...
      <DemoWithCode height="420px" code={customCode}>
        {#snippet blurb()}
          ...
          <p>
            ...
            panel out of the edge it sits on, so it gets a chevron (<code
              >&rsaquo;</code
            >/<code>&lsaquo;</code>); the sheet is conjured by a floating
            button, so it gets the menu glyph and closes with an x. Toggle
            overlay to swap between them.
          </p>
          <p>
            ...
            <code>--sidebar-expand</code>/<code>--sidebar-collapse</code>, so
            set those to change everything at once. Each has an
            <code>-image</code> twin taking a <code>url()</code> if you would rather
            ship SVGs.
          </p>
        {/snippet}
        {#snippet inputArea()}
          <Inline style="flex-wrap: wrap;">
            <FormItem>
      ...
      <DemoWithCode height="440px" code={scCode}>
        {#snippet blurb()}
          ...
          <p>
            ...
            <code>&lt;Page&gt;</code>. It needs exactly three things from its
            host: a
            ...
            <strong>flex parent</strong> (so the content lands beside it rather than
            below).
          </p>
          <p>
            ...
            <code>bg</code>/<code>fg</code>, and <code>right</code> to put the sidebar
            after the content.
          </p>
        {/snippet}
        {#snippet inputArea()}
        ...
        >
          ...
          <main style="padding: 0 1rem;">
            <p>
              Content, laid out beside the sidebar -- no <code>Page</code> in sight.
            </p>
          </main>
        </SidebarContainer>
      </DemoWithCode>
  ...
  <p>Content beside the sidebar</p>
      ...
      >
        {#snippet blurb()}
          ...
          <p>
            Nothing is special about <code>SidebarContainer</code> -- it is
            those three declarations and no more. Any element works if you
            supply them. The <code>container-type</code> is the one that bites: without
            it neither of the sidebar's container queries matches, and you get the
